...
const ICON_CAT_META = {
  0:  { vi: 'Không xác định',      en: 'Unknown',                color: '#94a3b8', bg: '#94a3b815' },
  1:  { vi: 'Tai nạn',             en: 'Accident',               color: '#dc2626', bg: '#dc262615' },
  2:  { vi: 'Sương mù',            en: 'Fog',                    color: '#64748b', bg: '#64748b15' },
  3:  { vi: 'Điều kiện nguy hiểm', en: 'Dangerous Conditions',   color: '#f97316', bg: '#f9731615' },
  4:  { vi: 'Mưa',                 en: 'Rain',                   color: '#3b82f6', bg: '#3b82f615' },
  5:  { vi: 'Đóng băng',           en: 'Ice',                    color: '#06b6d4', bg: '#06b6d415' },
  6:  { vi: 'Ùn tắc',             en: 'Jam',                    color: '#ef4444', bg: '#ef444415' },
  7:  { vi: 'Đóng làn',            en: 'Lane Closed',            color: '#7c3aed', bg: '#7c3aed15' },
  8:  { vi: 'Đường đóng',          en: 'Road Closed',            color: '#7c3aed', bg: '#7c3aed15' },
  9:  { vi: 'Thi công đường',      en: 'Road Works',             color: '#3b82f6', bg: '#3b82f615' },
  10: { vi: 'Gió mạnh',            en: 'Wind',                   color: '#64748b', bg: '#64748b15' },
  11: { vi: 'Ngập lụt',            en: 'Flooding',               color: '#0ea5e9', bg: '#0ea5e915' },
  14: { vi: 'Xe bị hỏng',          en: 'Broken Down Vehicle',    color: '#f97316', bg: '#f9731615' },
};

// Category 6 (Jam) sub-type refinement keyed by TomTom description text
const JAM_DESC_META = {
  'stationary traffic': { vi: 'Kẹt xe hoàn toàn', en: 'Stationary Traffic', color: '#ef4444', bg: '#ef444415' },
  'queuing traffic':    { vi: 'Xe xếp hàng',       en: 'Queuing Traffic',    color: '#f97316', bg: '#f9731615' },
  'slow traffic':       { vi: 'Giao thông chậm',   en: 'Slow Traffic',       color: '#eab308', bg: '#eab30815' },
};

const FALLBACK_META = { vi: 'Sự cố giao thông', en: 'Traffic Incident', color: '#f97316', bg: '#f9731615' };
...
function incidentMeta(iconCategory, description) {
  ...
  if (cat === 6 && description) {
    ...
    if (key.includes('stationar') || key.includes('kẹt')) return JAM_DESC_META['stationary traffic'];
...
export function buildIncidentPopupHTML(props, lang = 'vi') {
  ...
  const delayText = delayMin >= 1
    ? `+${delayMin} ${isEn ? 'min' : 'phút'}`
    : delaySec > 0 ? `+${delaySec}s` : '';

  const lengthM = Number(props?.length ?? 0);
  const lengthText = lengthM >= 1000
  ...
  ${hasBody ? `<div class="traffic-popup-body">
    ...
    ${lengthText || endT ? `<div class="traffic-popup-meta-row">
...
const CIRCLE_COLOR_EXPR = [
  ...
  1,  '#dc2626',  // Accident
  2,  '#64748b',  // Fog
  3,  '#f97316',  // Dangerous Conditions
  4,  '#3b82f6',  // Rain
  5,  '#06b6d4',  // Ice
  6,  ['match', ['get', 'description'],
    'stationary traffic', '#ef4444',
    'queuing traffic',    '#f97316',
    'slow traffic',       '#eab308',
    '#ef4444',
  ],
  7,  '#7c3aed',  // Lane Closed
  8,  '#7c3aed',  // Road Closed
  9,  '#3b82f6',  // Road Works
  10, '#64748b',  // Wind
  11, '#0ea5e9',  // Flooding
  14, '#f97316',  // Broken Down Vehicle
  '#f97316',
];

const CIRCLE_RADIUS_EXPR = [
  'interpolate', ['linear'], ['to-number', ['get', 'delay'], 0],
  0,   7,
  120, 9,
  300, 12,
  600, 15,
