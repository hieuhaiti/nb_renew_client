    ...
    <section className="bg-card mb-3 rounded-[10px] border-[0.5px] border-nature-border px-4 py-3.5">
      ...
      {(includes?.length > 0 || excludes?.length > 0) && (
        <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
          {includes?.length > 0 && (
            <div>
              <h3 className="mb-1.5 text-xs font-semibold text-nature">
              ...
              <ul className="space-y-1">
                {includes.map((item, i) => (
                  <li
                    key={i}
                    className="flex items-start gap-1.5 text-xs text-nature-muted-foreground"
                  >
                    <span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-nature" />
                    {item}
                  </li>
                ))}
              </ul>
